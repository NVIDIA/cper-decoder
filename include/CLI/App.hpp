...
class App
{
  ...
  protected:
    ...
    /// If set to true the name was automatically generated from the command
    /// line vs a user set name
    ...
    /// If set to true the subcommand is disabled and cannot be used, ignored
    /// for main app
    ...
    /// This is a function that generates a footer to put after all other
    ...
    /// This is a list of options which are required by this app, the list is
    /// not mutual, listed options do not need the subcommand not be
    ...
    /// Allow subcommand fallthrough, so that parent commands can collect
    /// commands after subcommand.  INHERITABLE
    ...
    /// specify that positional arguments come at the end of the argument
    /// sequence not inheritable
    ...
    /// Max number of subcommands allowed (parsing stops after this number). 0
    /// is unlimited INHERITABLE
        ...
        parent_(parent)
    {
        ...
        if (parent_ != nullptr)
        {
            ...
            validate_optional_arguments_ =
                parent_->validate_optional_arguments_;
            configurable_ = parent_->configurable_;
            allow_windows_style_options_ =
                parent_->allow_windows_style_options_;
  ...
  public:
    ...
    /// Set a callback for execution when all parsing and processing has
    /// completed
    ///
    ...
    App* name(std::string app_name = "")
    {
        if (parent_ != nullptr)
        {
            auto oname = name_;
            name_ = app_name;
            auto& res = _compare_subcommand_names(*this,
                                                  *_get_fallthrough_parent());
            if (!res.empty())
            {
                name_ = oname;
                throw(OptionAlreadyAdded(
                    app_name + " conflicts with existing subcommand names"));
            }
        }
        else
        {
    ...
    App* alias(std::string app_name)
    {
        ...
        if (parent_ != nullptr)
        {
            ...
            if (!res.empty())
            {
                aliases_.pop_back();
                throw(OptionAlreadyAdded(
                    "alias already matches an existing subcommand: " +
                    app_name));
            }
        }
        else
    ...
    /// Set the subcommand to be disabled by default, so on clear(), at the
    /// start of each parse it is disabled
    ...
    /// Set the subcommand to validate optional vector arguments before
    /// assigning
    ...
    App* ignore_case(bool value = true)
    {
        if (value && !ignore_case_)
        {
            ...
            if (!match.empty())
            {
                ...
                throw OptionAlreadyAdded(
                    "ignore case would cause subcommand name conflicts: " +
                    match);
            }
        }
        ignore_case_ = value;
    ...
    /// Allow windows style options, such as `/opt`. First matching short or
    /// long name used. Subcommands inherit value.
    ...
    /// Specify that the positional arguments are only at the end of the
    /// sequence
    ...
    App* ignore_underscore(bool value = true)
    {
        if (value && !ignore_underscore_)
        {
            ...
            if (!match.empty())
            {
                ...
                throw OptionAlreadyAdded(
                    "ignore underscore would cause subcommand name conflicts: " +
                    match);
            }
        }
    ...
    ///
    /// To use, create a variable with the expected type, and pass it in after
    /// the name. After start is called, you can use count to see if the value
    /// was passed, and the value will be initialized properly. Numbers,
    /// vectors, and strings are supported.
    ///
                       ...
                       std::function<std::string()> func = {})
    {
            ...
            std::end(options_))
        {
            ...
            option.reset(new Option(option_name, option_description,
                                    option_callback, this));

            // Set the default string capture function
            option->default_function(func);

            // For compatibility with CLI11 1.7 and before, capture the default
            // string here
            if (defaulted)
        ...
        // we know something matches now find what it is so we can produce more
        // error information
        ...
        // this line should not be reached the above loop should trigger the
        // throw
        throw(OptionAlreadyAdded(
    ...
    {
        auto fun =
            [&variable](const CLI::results_t& res) { // comment for spacing
            return detail::lexical_conversion<AssignTo, ConvertTo>(res,
                                                                   variable);
        };

        Option* opt = add_option(option_name, fun, option_description, false,
                                 [&variable]() {
            return CLI::detail::checked_to_string<AssignTo, ConvertTo>(
                variable);
        });
        opt->type_name(detail::type_name<ConvertTo>());
        // these must be actual lvalues since (std::max) sometimes is defined in
        // terms of references and references to structs used in the evaluation
        // can be temporary so that would cause issues.
                                 ...
                                 std::string option_description = "")
    {
        auto fun =
            [&variable](const CLI::results_t& res) { // comment for spacing
            return detail::lexical_conversion<AssignTo, AssignTo>(res,
                                                                  variable);
        };
                                ...
                                std::string option_description = "")
    {
        auto fun = [func](const CLI::results_t& res) {
            ArgType variable;
            bool result =
                detail::lexical_conversion<ArgType, ArgType>(res, variable);
            if (result)
            {
                func(variable);
            }
    ...
    {
        // take flag_description by const reference otherwise add_flag tries to
        // assign to help_description
    ...
    /// Set a version flag and version display string, replace the existing one
    /// if present
                                 ...
                                 "Display program version information and exit")
    {
        ...
        if (!flag_name.empty())
        {
            version_ptr_ = add_flag_callback(flag_name, [versionString]() {
                                 ...
                                 "Display program version information and exit")
    {
        ...
        if (!flag_name.empty())
        {
            version_ptr_ = add_flag_callback(flag_name, [vfunc]() {
  ...
  public:
    ...
    {
        return _add_flag_internal(flag_name, CLI::callback_t(),
                                  flag_description);
    }
    ...
    template <typename T,
              enable_if_t<!detail::is_mutable_container<T>::value &&
                              !std::is_const<T>::value &&
                              !std::is_constructible<std::function<void(int)>,
                                                     T>::value,
                          detail::enabler> = detail::dummy>
    Option* add_flag(std::string flag_name,
    ...
    template <typename T,
              enable_if_t<!std::is_assignable<
                              std::function<void(std::int64_t)>&, T>::value,
                          detail::enabler> = detail::dummy>
    Option* add_flag(std::string flag_name,
                        ...
                        std::string subcommand_description = "")
    {
        if (!subcommand_name.empty() &&
            !detail::valid_name_string(subcommand_name))
        {
            ...
            for (auto c : subcommand_name)
            {
                if (!detail::valid_later_char(c))
                {
                    throw IncorrectConstruction(
                        std::string(
                            "Subcommand name contains invalid character ('") +
                        c +
    ...
    App* add_subcommand(CLI::App_p subcom)
    {
        ...
        auto ckapp = (name_.empty() && parent_ != nullptr)
                         ? _get_fallthrough_parent()
                         : this;
    ...
    /// Check to see if a subcommand is part of this command and get a
    /// shared_ptr to it
    ...
    std::size_t count_all() const
    {
        ...
        if (!get_name().empty())
        { // for named subcommands add the number of times the subcommand was
          // called
            cnt += parsed_;
        }
        return cnt;
    ...
    /// The argumentless form of require subcommand requires 1 or more
    /// subcommands
    ...
    /// The argumentless form of require option requires 1 or more options be
    /// used
    App* require_option()
    {
    ...
    void parse(std::string commandline, bool program_name_included = false)
    {
        ...
        // the next section of code is to deal with quoted arguments after an
        // '=' or ':' for windows like operations
        if (!commandline.empty())
        {
            commandline = detail::find_and_modify(commandline, "=",
        ...
        args.erase(std::remove(args.begin(), args.end(), std::string{}),
                   args.end());
    ...
    void parse(std::vector<std::string>& args)
    {
        ...
        // running parse after an error is thrown, even by _validate or
        // _configure.
        parsed_ = 1;
        _validate();
        _configure();
    ...
    /// Get a subcommand pointer list to the currently selected subcommands
    /// (after parsing by default, in command line order; use parsed = false to
    /// get the original definition list.)
    std::vector<App*> get_subcommands() const
    {
        return parsed_subcommands_;
    }

    /// Get a filtered subcommand pointer list from the original definition
    /// list. An empty function will provide all subcommands (const)
    ...
    /// Get a filtered subcommand pointer list from the original definition
    /// list. An empty function will provide all subcommands
    ...
    {
        auto iterator = std::find(std::begin(need_options_),
                                  std::end(need_options_), opt);
    ...
    {
        return config_formatter_->to_config(this, default_also,
                                            write_description, "");
    }
    ...
    {
        // This is safer as a dynamic_cast if we have RTTI, as Config ->
        // ConfigBase
    ...
    /// Get the parent of this subcommand (or nullptr if main app) (const
    /// version)
        ...
        if (recurse)
        {
            if (!allow_extras_)
            {
                for (const auto& sub : subcommands_)
                {
                    ...
                    {
                        for (const std::pair<detail::Classifier, std::string>&
                                 miss : sub->missing_)
                        {
                            miss_list.push_back(std::get<1>(miss));
                        }
    ...
    /// This returns the missing options in a form ready for processing by
  ...
  protected:
    ...
    ///
    /// Currently checks to see if multiple positionals exist with unlimited
    /// args and checks if the min and max options are feasible
    void _validate() const
    {
        ...
        if (pcount > 1)
        {
            auto pcount_req = std::count_if(std::begin(options_),
                                            std::end(options_),
                                            [](const Option_p& opt) {
                return opt->get_items_expected_max() >=
                           detail::expected_max_vector_size &&
    ...
    /// manage the automatic enable or disable makes sure parent is set
    /// correctly
    void _configure()
    {
        ...
        for (const App_p& app : subcommands_)
        {
            ...
            if (app->name_.empty())
            {
                app->fallthrough_ = false; // make sure fallthrough_ is false to
                                           // prevent infinite loop
                app->prefix_command_ = false;
            }
            // make sure the parent is set to be this object in preparation for
            // parse
            app->parent_ = this;
            app->_configure();
        }
    ...
    {
        pre_callback();
        // in the main app if immediate_callback_ is set it runs the main
        // callback before the used subcommands
    ...
    void _process_config_file()
    {
        if (config_ptr_ != nullptr)
        {
            ...
            for (auto rit = config_files.rbegin(); rit != config_files.rend();
                 ++rit)
            {
    ...
    ///
    /// The flags allow recursive calls to remember if there was a help flag on
    /// a parent.
                             ...
                             bool trigger_all_help = false) const
    {
        ...
        // If there were parsed subcommands, call those. First subcommand wins
        ...
        if (!parsed_subcommands_.empty())
        {
            ...
            // Only the final subcommand should call for help. All help wins
            // over help.
        }
    ...
    void _process_requirements()
    {
        ...
        // Max error cannot occur, the extra subcommand will parse as an
        // ExtrasError or a remaining item.

        // run this loop to check how many unnamed subcommands were actually
        // used since they are considered options from the perspective of an App
        for (App_p& sub : subcommands_)
        {
            if (sub->disabled_)
                continue;
            ...
            (require_option_max_ > 0 && require_option_max_ < used_options))
        {
            ...
            {
                option_list += "," +
                               detail::join(subc_list, [](const App* app) {
                    return app->get_display_name();
                });
            }
            throw RequiredError::Option(require_option_min_,
        ...
        for (App_p& sub : subcommands_)
        {
            ...
            if (sub->name_.empty() && sub->required_ == false)
            {
                ...
                {
                    if (require_option_min_ > 0 &&
                        require_option_min_ <= used_options)
                    {
                        continue;
                        // if we have met the requirement and there is nothing
                        // in this option group skip checking requirements
                    }
                    if (require_option_max_ > 0 &&
                        used_options >= require_option_min_)
                    {
                        continue;
                        // if we have met the requirement and there is nothing
                        // in this option group skip checking requirements
    ...
    void _process()
    {
        try
        {
            ...
            // processed until later in the process to allow for help, version
            // and other errors to generate first.
        ...
        {
            // callbacks and help_flags can generate exceptions which should
            // take priority over the config file error if one exists.
            _process_callbacks();
            _process_help_flags();
            throw;
        }
    ...
    void _parse(std::vector<std::string>& args)
    {
        ...
        if (parent_ == nullptr)
        {
            ...
            // Convert missing (pairs) to extras (string only) ready for
            // processing in another app
    ...
    void _parse(std::vector<std::string>&& args)
    {
        // this can only be called by the top level in which case parent ==
        // nullptr by definition operation is simplified
        increment_parsed();
        _trigger_pre_parse(args.size());
        bool positional_only = false;
    ...
    /// matching it Returns true if it managed to find the option, if false
    /// you'll need to remove the arg manually.
    ...
    bool _parse_single_config(const ConfigItem& item, std::size_t level = 0)
    {
        ...
        if (op == nullptr)
        {
            ...
            if (get_allow_config_extras() == config_extras_mode::capture)
                ...
                missing_.emplace_back(detail::Classifier::NONE,
                                      item.fullname());
            return false;
        }
    ...
    {
        bool retval = true;
        detail::Classifier classifier = positional_only
    ...
    std::size_t _count_remaining_positionals(bool required_only = false) const
    {
        ...
        {
            if (opt->get_positional() &&
                (!required_only || opt->get_required()))
            {
                if (opt->get_items_expected_min() > 0 &&
                    static_cast<int>(opt->count()) <
                        opt->get_items_expected_min())
                {
                    retval += static_cast<std::size_t>(
                                  opt->get_items_expected_min()) -
                              opt->count();
                }
            }
    ...
    bool _has_remaining_positionals() const
    {
        for (const Option_p& opt : options_)
        {
            if (opt->get_positional() && ((static_cast<int>(opt->count()) <
                                           opt->get_items_expected_min())))
            {
                return true;
            }
                           ...
                           bool haltOnSubcommand)
    {
        ...
        {
            // deal with the case of required arguments at the end which should
        ...
        /// now try one last gasp at subcommands that have been executed before,
        /// go to root app and try to find a subcommand in a broader way, if one
        /// exists let the parent deal with it
        auto parent_app = (parent_ != nullptr) ? _get_fallthrough_parent()
                                               : this;
                    ...
                    detail::Classifier current_type)
    {
        ...
        if (op_ptr == std::end(options_))
        {
            ...
            // don't capture missing if this is a nameless subcommand and
            // nameless subcommands can't fallthrough
            if (parent_ != nullptr && name_.empty())
            {
                return false;
            }
            ...
            if (parent_ != nullptr && fallthrough_)
                return _get_fallthrough_parent()->_parse_arg(args,
                                                             current_type);
        ...
        int min_num = (std::min)(op->get_type_size_min(),
                                 op->get_items_expected_min());
        int max_num = op->get_items_expected_max();
        // check container like options to limit the argument size to a single
        // type if the allow_extra_flags argument is set. 16 is somewhat
        // arbitrary (needs to be at least 4)
        ...
        { // we allow optional arguments
            ...
            if (!args.empty() &&
                ...
                args.pop_back();
            // optional flag that didn't receive anything now get the default
            // value
        ...
        // if we only partially completed a type then add an empty string if
        // allowed for later processing
        if (min_num > 0 && (collected % op->get_type_size_max()) != 0)
        {
            ...
            {
                throw ArgumentMismatch::PartialType(op->get_name(),
                                                    op->get_type_size_min(),
                                                    op->get_type_name());
            }
        }
        if (op->get_trigger_on_parse())
    ...
    const std::string& _compare_subcommand_names(const App& subcom,
                                                 const App& base) const
    {
        ...
        for (auto& subc : base.subcommands_)
        {
            if (subc.get() != &subcom)
            {
                ...
                // this loop is needed in case of ignore_underscore or
                // ignore_case on one but not the other
                ...
                // if the test subcommand is an option group we need to check
                // deeper
                if (subcom.get_name().empty())
                {
    ...
    void _move_to_missing(detail::Classifier val_type, const std::string& val)
    {
        ...
        // allow extra arguments to be places in an option group if it is
        // allowed there
        for (auto& subc : subcommands_)
        {
  ...
  public:
    ...
    void _move_option(Option* opt, App* app)
    {
        ...
        if (iterator != std::end(options_))
        {
            ...
            {
                throw OptionAlreadyAdded("option was not located: " +
                                         opt->get_name());
            }
        }
        else
...
class Option_group : public App
{
  public:
    Option_group(std::string group_description, std::string group_name,
                 App* parent) : App(std::move(group_description), "", parent)
    {
        group(group_name);
...
inline void retire_option(App* app, const std::string& option_name)
{
    ...
    auto opt2 = app->add_option(option_name,
                                "option has been retired and has no effect")
                    ->type_name("RETIRED")
                    ->expected(0, 1)
                    ->default_str("RETIRED");
...
inline std::string simple(const App* app, const Error& e)
{
    ...
    if (!names.empty())
        header += "Run with " + detail::join(names, " or ") +
...
{
    std::string header = std::string("ERROR: ") + e.get_name() + ": " +
                         e.what() + "\n";
    header += app->help();
    return header;
}
