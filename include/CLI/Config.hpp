...
inline std::string ini_join(const std::vector<std::string>& args,
                            char sepChar = ',', char arrayStart = '[',
                            char arrayEnd = ']', char stringQuote = '"',
                            char characterQuote = '\'')
{
    std::string joined;
...
inline std::vector<std::string> generate_parents(const std::string& section,
                                                 std::string& name,
...
inline void checkParentSegments(std::vector<ConfigItem>& output,
                                ...
                                char parentSeparator)
{
    ...
    if (!output.empty() && output.back().name == "--")
    {
        ...
        {
            std::size_t common = 0;
            std::size_t mpair = (std::min)(output.back().parents.size(),
                                           parents.size() - 1);
    ...
    ConfigBase::from_config(std::istream& input) const
{
    ...
    bool isDefaultArray = (arrayStart == '[' && arrayEnd == ']' &&
                           arraySeparator == ',');
    bool isINIArray = (arrayStart == '\0' || arrayStart == ' ') &&
                      arrayStart == arrayEnd;
    ...
    while (getline(input, line))
    {
        ...
        // lines have to be at least 3 characters to have any meaning to CLI
        // just skip the rest
        if (len < 3)
        {
            continue;
        ...
        if (line.front() == '[' && line.back() == ']')
        {
            if (currentSection != "default")
            {
                // insert a section end which is just an empty items_buffer
                output.emplace_back();
                output.back().parents = detail::generate_parents(
                    currentSection, name, parentSeparatorChar);
                output.back().name = "--";
            }
            ...
            {
                currentSection =
                    currentSection.substr(1, currentSection.size() - 2);
            }
    ...
    if (currentSection != "default")
    {
        ...
        std::string ename;
        output.emplace_back();
        output.back().parents = detail::generate_parents(currentSection, ename,
                                                         parentSeparatorChar);
        output.back().name = "--";
...
inline std::string ConfigBase::to_config(const App* app, bool default_also,
        ...
        for (const Option* opt : app->get_options({}))
        {
            ...
            if (opt->get_configurable())
            {
                ...
                std::string value = detail::ini_join(
                    opt->reduced_results(), arraySeparator, arrayStart,
                ...
                if (value.empty() && default_also)
                {
                    if (!opt->get_default_str().empty())
                    {
                        value = detail::convert_arg_for_ini(
                            opt->get_default_str(), stringQuote,
                            characterQuote);
                    }
                ...
                if (!value.empty())
                {
                    if (write_description && opt->has_description())
                    {
                        out << '\n';
                        out << commentLead
                            << detail::fix_newlines(commentLead,
                                                    opt->get_description())
                            << '\n';
                    }
    ...
    for (const App* subcom : subcommands)
    {
        if (!subcom->get_name().empty())
        {
            if (subcom->get_configurable() && app->got_subcommand(subcom))
            {
                ...
                {
                    std::string subname = app->get_name() +
                                          parentSeparatorChar +
                                          subcom->get_name();
