...
class OptionBase
{
  ...
  public:
    ...
    /// Return true if this will automatically capture the default value for
    /// help printing
...
class Option : public OptionBase<Option>
{
  ...
  protected:
    ...
    /// A human readable default value, either manually set, captured, or
    /// captured by default
    std::string default_str_{};

    /// If given, replace the text that describes the option type and usage in
    /// the help text
    ...
    /// flag indicating a separator needs to be injected after each argument
    /// call
    bool inject_separator_{false};
    /// flag indicating that the option should trigger the validation and
    /// callback chain on each result when loaded
    ...
    /// Making an option by hand is not defined, it must be made by the App
    /// class
  ...
  public:
    ...
    /// This bool operator returns true if any arguments were passed or the
    /// option callback is forced
    ...
    /// Set the value of trigger_on_parse which specifies that the option
    /// callback should be triggered on every parse
    ...
    Validator* get_validator(int index)
    {
        ...
        {
            return &(validators_[static_cast<decltype(validators_)::size_type>(
                index)]);
        }
    ...
    /// Remove needs link from an option. Returns true if the option really was
    /// in the needs list.
    ...
    Option* excludes(Option* opt)
    {
        ...
        // (Mostly to allow both directions to be excluded by user, even though
    ...
    {
        auto iterator = std::find(std::begin(excludes_), std::end(excludes_),
                                  opt);
    ...
    /// Ignore case
    ///
    /// The template hides the fact that we don't have the definition of App
    /// yet. You are never expected to add an argument to the template here.
    template <typename T = App>
    ...
    ) const
    {
        ...
        if (all_options)
        {
            ...
            /// The all list will never include a positional unless asked or
    ...
    const std::string& matching_name(const Option& other) const
    {
        ...
        { // We need to do the inverse, in case we are ignore_case or ignore
          // underscore
    ...
    bool check_name(const std::string& name) const
    {
        ...
        if (!envname_.empty())
        {
            // this needs to be the original since envname_ shouldn't match on
            // case insensitivity
    ...
    {
        return (detail::find_member(std::move(name), snames_, ignore_case_) >=
                0);
    }
    ...
    std::string get_flag_value(const std::string& name,
                               std::string input_value) const
    {
        ...
        if (disable_flag_override_)
        {
            if (!((input_value.empty()) || (input_value == emptyString)))
            {
                auto default_ind = detail::find_member(
                    name, fnames_, ignore_case_, ignore_underscore_);
                if (default_ind >= 0)
                {
                    // We can static cast this to std::size_t because it is more
                    // than 0 in this block
                    if (default_flag_values_[static_cast<std::size_t>(
                                                 default_ind)]
                            .second != input_value)
                    {
        ...
        auto ind = detail::find_member(name, fnames_, ignore_case_,
                                       ignore_underscore_);
        if ((input_value.empty()) || (input_value == emptyString))
        {
            if (flag_like_)
            {
                return (ind < 0)
                           ? trueString
                           : default_flag_values_[static_cast<std::size_t>(ind)]
                                 .second;
            }
            else
            {
    ...
    void results(T& output) const
    {
        ...
        {
            const results_t& res = (proc_results_.empty()) ? results_
                                                           : proc_results_;
    ...
    /// Capture the default value from the original value (if it can be
    /// captured)
    ...
    /// Set the default value string representation (does not change the
    /// contained value)
    ...
    /// appropriate to set the value into the bound value only available for
    ...
    Option* default_val(const X& val)
    {
        ...
        try
        {
            ...
            {
                run_callback(); // run callback sets the state, we need to reset
                                // it again
                current_option_state_ = option_state::parsing;
            }
            else
  ...
  private:
        ...
        if (!validators_.empty())
        {
            ...
            { // in this context index refers to the index in the type
                    ...
                    multi_option_policy_ == CLI::MultiOptionPolicy::TakeLast)
                {
                    ...
                    index = get_items_expected_max() -
                            static_cast<int>(res.size());
                }

                for (std::string& result : res)
                {
                    if (detail::is_separator(result) &&
                        type_size_max_ != type_size_min_ && index >= 0)
                    {
                        index = 0; // reset index for variable size chunks
                        continue;
                    }
                    auto err_msg = _validate(
                        result,
                        (index >= 0) ? (index % type_size_max_) : index);
                    if (!err_msg.empty())
    ...
    */
    void _reduce_results(results_t& res, const results_t& original) const
    {
            ...
            case MultiOptionPolicy::TakeLast:
            {
                ...
                std::size_t trim_size = std::min<std::size_t>(
                    static_cast<std::size_t>(
                        std::max<int>(get_items_expected_max(), 1)),
                    original.size());
        ...
        // this check is to allow an empty vector in certain circumstances but
        // not if expected is not zero.
