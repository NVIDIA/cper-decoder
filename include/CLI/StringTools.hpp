...
inline std::ostream& format_aliases(std::ostream& out,
    ...
    while (loc != std::string::npos)
    {
        ...
        if ((finish != std::string::npos) && (flags[finish] == '}'))
        {
            flags.erase(flags.begin() + static_cast<std::ptrdiff_t>(loc),
                        flags.begin() + static_cast<std::ptrdiff_t>(finish) +
                            1);
        }
    ...
    if (ignore_case)
    {
        if (ignore_underscore)
        {
            ...
            it = std::find_if(std::begin(names), std::end(names),
                ...
                return detail::to_lower(
                           detail::remove_underscore(local_name)) == name;
            });
        }
        else
        {
...
inline std::vector<std::string> split_up(std::string str, char delimiter = '\0')
{
    ...
    auto find_ws = [delimiter](char ch) {
        return (delimiter == '\0')
                   ? (std::isspace<char>(ch, std::locale()) != 0)
                   : (ch == delimiter);
    };
    trim(str);
    ...
    while (!str.empty())
    {
        ...
        if (embeddedQuote)
        {
            output.back() = find_and_replace(output.back(),
                                             std::string("\\") + keyChar,
    ...
    if ((next == '\"') || (next == '\'') || (next == '`'))
    {
        ...
        if (astart != std::string::npos)
        {
            if (str[astart] == ((str[offset] == '=') ? '-' : '/'))
                str[offset] = ' '; // interpret this as a space so the split_up
                                   // works properly
        }
    }
    return offset + 1;
