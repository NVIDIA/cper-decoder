...
{
    static const bool value = is_shared_ptr<T>::value ||
...
template <typename T>
struct is_wrapper<T,
                  conditional_t<false, void_t<typename T::value_type>, void>> :
...
template <typename T>
struct type_count<T,
                  typename std::enable_if<is_mutable_container<T>::value>::type>
{
                               ...
                               !is_tuple_like<T>::value>::type>
{
    static constexpr int value{
        subtype_count_min<typename T::value_type>::value};
};
             ...
             !is_direct_constructible<T, int>::value))>::type>
{
    ...
    // which could lead to issues so there are two variants of the condition,
    // the first isolates things with a type size >=2 mainly to get tuples on
    // Xcode with the exception of wrappers, the second is the main one and just
    // separating out those cases that are caught by other object
    // classifications
};
...
{
    std::string str =
        std::string(type_name<typename std::decay<
...
bool integral_conversion(const std::string& input, T& output) noexcept
{
    ...
    if (val == (input.c_str() + input.size()))
    {
        output = (output_sll < 0) ? static_cast<T>(0)
                                  : static_cast<T>(output_sll);
...
bool lexical_cast(const std::string& input, T& output)
{
    ...
    {
        // if the number is out of the range of a 64 bit value then it is still
        // a number and for this purpose is still valid all we care about the
        // sign
        output = (input[0] != '-');
        return true;
    }
...
#endif
        // with Atomic<XX> this could produce a warning due to the conversion
        // but if atomic gets here it is an old style so will most likely still
        // work
        output = val;
#ifdef _MSC_VER
#pragma warning(pop)
    ...
    // fail over from_stream is tested elsewhere an not relevant for coverage
    // here
    return from_stream(input, output);
    // LCOV_EXCL_STOP
}
...
{
    ConvertTo val{};
    bool parse_result = (!input.empty()) ? lexical_cast<ConvertTo>(input, val)
                                         : true;
    if (parse_result)
    {
        output = val;
...
{
    ConvertTo val{};
    bool parse_result = input.empty() ? true
                                      : lexical_cast<ConvertTo>(input, val);
    if (parse_result)
    {
        output = AssignTo(val); // use () form of constructor to allow some
                        ...
                        AssignTo& output)
{
    ...
    {
        retval = retval &&
                 lexical_assign<decltype(v2), decltype(v2)>(strings[1], v2);
    }
    if (retval)
    {
                        ...
                        AssignTo& output)
{
    if (strings.size() >= 2 && !strings[1].empty())
    {
        ...
        auto worked = detail::lexical_cast(strings[0], x) &&
                      detail::lexical_cast(str1, y);
        if (worked)
        {
                        ...
                        AssignTo& output)
{
    ...
    for (const auto& elem : strings)
    {
        output.emplace_back();
        retval = retval &&
                 lexical_assign<typename AssignTo::value_type, ConvertTo>(
                     elem, output.back());
    }
...
{
    bool retval = true;
    using ConvertToElement = typename std::conditional<
        is_tuple_like<ConvertTo>::value,
        typename std::tuple_element<I, ConvertTo>::type, ConvertTo>::type;
    if (!strings.empty())
    {
        retval = retval && tuple_type_conversion<
                               typename std::tuple_element<I, AssignTo>::type,
...
bool lexical_conversion(std::vector<std::string> strings, AssignTo& output)
{
    ...
    while (!strings.empty())
    {
        ...
        bool retval = tuple_type_conversion<decltype(v1), decltype(v1)>(strings,
                                                                        v1);
        if (!strings.empty())
        {
            retval = retval &&
                     tuple_type_conversion<decltype(v2), decltype(v2)>(strings,
                                                                       v2);
        }
        if (retval)
        {
                        ...
                        AssignTo& output)
{
    ...
    while (ii < ii_max)
    {
        ...
        if (icount == xcm || is_separator(temp.back()) || ii == ii_max)
        {
            ...
            retval = retval &&
                     lexical_conversion<typename AssignTo::value_type,
