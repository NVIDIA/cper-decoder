...
class Validator
{
  ...
  public:
    ...
    /// Get a boolean if the validator is allowed to modify the input returns
    /// true if it can modify the input
    bool get_modifying() const
    {
        return !non_modifying_;
    }

    /// Combining validators is a new validator. Type comes from left validator
    /// if function, otherwise only set if the same.
    Validator operator&(const Validator& other) const
    {
        ...
        const std::function<std::string(std::string & filename)>& f2 =
            other.func_;
    ...
    Validator operator!() const
    {
        ...
        newval.func_ = [f1, dfunc1](std::string& test) -> std::string {
            ...
            if (s1.empty())
            {
                return std::string("check ") + dfunc1() +
  ...
  public:
    IPV4Validator() : Validator("IPV4")
    {
        func_ = [](std::string& ip_addr) {
            ...
            {
                return std::string(
                           "Invalid IPV4 address must have four parts (") +
                       ip_addr + ')';
            }
            int num;
  ...
  public:
        ...
        Validator(validator_name)
    {
        func_ = [](std::string& input_string) {
            ...
            {
                return std::string("Failed parsing ") + input_string +
...
class Range : public Validator
{
  public:
    ...
    ///
    /// Note that the constructor is templated, but the struct is not, so C++17
    /// is not needed to provide nice syntax for Range(a,b).
    template <typename T>
        ...
        Validator(validator_name)
    {
        if (validator_name.empty())
        {
            ...
            out << detail::type_name<T>() << " in [" << min_val << " - "
                << max_val << "]";
            description(out.str());
        }
...
/// Check for a positive valued number (val>0.0), min() her is the smallest
/// positive number
...
std::string generate_set(const T& set)
{
    ...
    std::string out(1, '{');
    out.append(
        detail::join(detail::smart_deref(set), [](const iteration_type_t& v) {
...
std::string generate_map(const T& map, bool key_only = false)
{
    ...
    out.append(detail::join(detail::smart_deref(map),
        ...
        if (!key_only)
        {
            res.append("->");
            res +=
                detail::to_string(detail::pair_adaptor<element_t>::second(v));
        }
        return res;
    }, ","));
    out.push_back('}');
    ...
    overflowCheck(const T& a, const T& b)
{
    ...
    {
        return ((std::numeric_limits<T>::min)() / (std::abs)(a) >
                -(std::abs)(b));
    }
}
...
class IsMember : public Validator
{
  public:
    ...
    explicit IsMember(T set, F filter_function)
    {
        ...
        // ::value_type if the type does not have first_type and second_type,
        // these are both value_type
        using element_t =
            typename detail::element_type<T>::type; // Removes (smart) pointers
                                                    // if needed
        using item_t = typename detail::pair_adaptor<
            element_t>::first_type; // Is value_type if not a map

        using local_item_t =
            typename IsMemberType<item_t>::type; // This will convert bad types
                                                 // to good ones (const char *
                                                 // to std::string)

        // Make a local copy of the filter function, using a std::function if
        // not one already
        std::function<local_item_t(local_item_t)> filter_fn = filter_function;

        // This is the type name for help, it will take the current version of
        // the set contents
        desc_function_ = [set]() {
        ...
        // It stores a copy of the set pointer-like, so shared_ptr will stay
        // alive
        func_ = [set, filter_fn](std::string& input) {
            ...
            if (res.first)
            {
                // Make sure the version in the input string is identical to the
                // one in the set
                if (filter_fn)
                {
  ...
  public:
    ...
    explicit Transformer(T mapping, F filter_function)
    {
        ...
        using item_t = typename detail::pair_adaptor<
            element_t>::first_type;              // Is value_type if not a map
        using local_item_t =
            typename IsMemberType<item_t>::type; // Will convert bad types to
        ...
        func_ = [mapping, filter_fn](std::string& input) {
            ...
            {
                return std::string();
                // there is no possible way we can match anything in the mapping
                // if we can't convert so just return
            }
            if (filter_fn)
            {
                b = filter_fn(b);
  ...
  public:
    ...
    explicit CheckedTransformer(T mapping, F filter_function)
    {
        ...
        auto tfunc = [mapping]() {
            ...
            out += detail::join(detail::smart_deref(mapping),
        ...
        func_ = [mapping, tfunc, filter_fn](std::string& input) {
            ...
            {
                auto output_string = detail::value_string(
                    detail::pair_adaptor<element_t>::second(v));
    ...
    split_program_name(std::string commandline)
{
           ...
           path_type::file)
    {
        ...
        {
            // if we have reached the end and haven't found a valid file just
            // assume the first argument is the program name
                ...
                commandline[0] == '`')
            {
                ...
                if (end != std::string::npos)
                {
                    ...
                    if (embeddedQuote)
                    {
                        vals.first = find_and_replace(
                            vals.first, std::string("\\") + keyChar,
    ...
    vals.second = (esp != std::string::npos) ? commandline.substr(esp + 1)
                                             : std::string{};
    ltrim(vals.second);
    return vals;
}
